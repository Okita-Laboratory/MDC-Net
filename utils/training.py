...
from models.MotionTransformer.transformer import EMA
from utils import *
...
class Trainer:
    ...
    def before_train(self):
        ...
        self.criterion = nn.MSELoss()  # the only one loss function in paper
    ...
    def run_train_step(self):
        for traj_np in self.generator_train:
            with torch.no_grad():
                ...
                traj_np = traj_np[..., 1:, :].reshape([traj_np.shape[0], self.cfg.t_his + self.cfg.t_pred + self.cfg.t_fut, -1])
                # 原始轨迹
            ...
            # 对原始轨迹的离散余弦变换应用噪声    得到噪声样本y_t和真实噪声noise
            y_t, noise = self.diffusion.noise_motion(traj_dct, t)
            # 对填充后轨迹的离散余弦变换应用transformer  预测噪声
            # TransLinear
            predicted_noise = self.model(y_t, t, mod=traj_dct_mod)
            # print('predicted_noise.shape:', predicted_noise.shape)
            # print('noise.shape:', noise.shape)
            # MSELoss  预测的噪声和真实噪声
            L2_loss = self.criterion(predicted_noise, noise)
            loss = L2_loss
    ...
    def after_train_step(self):
        ...
        if self.iter % self.cfg.save_gif_interval == 0:
            ...
            render_animation(skeleton=self.dataset['train'].skeleton,
                             poses_generator=pose_gen,
                             algos=['HumanMAC'],
                             t_hist=self.cfg.t_his,
                             t_predt=self.cfg.t_fut,
                             ncol=4,
    ...
    def run_val_step(self):
        for traj_np in self.generator_val:
            with torch.no_grad():
                # (N, t_his + t_pre + t_fut, joints, 3) -> (N, t_his + t_pre + t_fut, 3 * (joints - 1))
                # discard the root joint and combine xyz coordinate
                traj_np = traj_np[..., 1:, :].reshape([traj_np.shape[0], self.cfg.t_his + self.cfg.t_pred + self.cfg.t_fut, -1])
                traj = tensor(traj_np, device=self.cfg.device, dtype=self.cfg.dtype)
    ...
    def after_val_step(self):
        ...
        if self.iter % self.cfg.save_gif_interval == 0:
            if self.cfg.ema is True:
                ...
                render_animation(skeleton=self.dataset['test'].skeleton,
                                 poses_generator=pose_gen,
                                 algos=['HumanMAC'],
                                 t_hist=self.cfg.t_his,
                                 t_predt=self.cfg.t_pred,
                                 ncol=4,
