...
from models.MotionTransformer.transformer import MotionTransformer
from models.MyModel.MyModel import MyModel
from models.MDM.mdm import MDM
from models.FlowMDM.FlowMDM import FlowMDM
...
def create_model_and_diffusion(cfg):
    ...
    if cfg.model == 'MyModel':
    ...
    elif cfg.model == 'MotionTransformer':
        ...
        raise ValueError(f'{cfg.model} is not available! Only "Unet" and "MotionTransformer". Please check cfg.')
...
def dataset_split(cfg):
    ...
    dataset = dataset_cls('train', cfg.t_his, cfg.t_pred, cfg.t_fut, actions='all')
    dataset_test = dataset_cls('test', cfg.t_his, cfg.t_pred, cfg.t_fut, actions='all')

    dataset_cls_multi = DatasetH36M_multi if cfg.dataset == 'h36m' else DatasetHumanEva_multi
    dataset_multi_test = dataset_cls_multi('test', cfg.t_his, cfg.t_pred, cfg.t_fut,
...
def get_multimodal_gt_full(logger, dataset_multi_test, args, cfg):
    ...
    gt_group = all_data[:, cfg.t_his:(cfg.t_his + cfg.t_pred + 1), :]
    ...
    for i in range(pd.shape[0]):
        ...
        traj_gt_arr.append(all_data[ind][:, cfg.t_his:(cfg.t_his + cfg.t_pred + 1), :])
...
def sample_preprocessing(traj, cfg, mode):
    ...
    output: a dict for specific mode,
            ...
            traj_dct_mod
    """
    if mode == 'joint' or mode == 'freedom':
        n = traj.shape[0]
        traj_joint_his = traj[:, 0:cfg.t_his, :]
        traj_joint_fut = traj[:, (cfg.t_his + cfg.t_pred):(cfg.t_pred + cfg.t_his + cfg.t_fut), :]
        direct_current = traj[:, (cfg.t_his + cfg.t_pred), :].unsqueeze(1).repeat(1, cfg.t_pred,
                                                                              1)
        traj_joint = torch.cat([traj_joint_his, direct_current, traj_joint_fut], dim=1)
        traj_joint = traj_joint[0].unsqueeze(0).repeat(n, 1, 1)
        # traj_switch = torch.roll(traj_switch, 1, 0) # make the target traj be various
        mask = torch.zeros([n, cfg.t_his + cfg.t_pred + cfg.t_fut, traj.shape[-1]]).to(cfg.device)
        for i in range(0, cfg.t_his):
            mask[:, i, :] = 1

        mask_joint = torch.zeros([n, cfg.t_his + cfg.t_pred + cfg.t_fut, traj.shape[-1]]).to(cfg.device)
        for i in range(cfg.t_his + cfg.t_pred, cfg.t_his + cfg.t_pred + cfg.t_fut):
            mask_joint[:, i, :] = 1
        """
            生成的动作晃来晃去的罪魁祸首. 
            mask_joint应该表示转变后的动作，只把future motion置1。但是我在这把history motion也置成了1.
        """
        # for i in range(0, cfg.t_his):
        #     mask_joint[:, i, :] = 1
        ...
        return {'traj_joint': traj_joint,
                'mask_joint': mask_joint,
                'mask': mask,
                'sample_num': n,
                'mode': 'joint'}, traj_dct, traj_dct_mod

    elif mode == 'switch':
        n = traj.shape[0]
        traj_switch = traj[:, (cfg.t_his + cfg.t_pred):(cfg.t_pred + cfg.t_his + cfg.t_fut), :]
        direct_current = traj[:, (cfg.t_his + cfg.t_pred), :].unsqueeze(1).repeat(1, cfg.t_his + cfg.t_pred,
                                                                              1)
        ...
        mask_end = torch.zeros([n, cfg.t_his + cfg.t_pred + cfg.t_fut, traj.shape[-1]]).to(cfg.device)
        for i in range(cfg.t_his + cfg.t_pred, cfg.t_his + cfg.t_pred + cfg.t_fut):
            mask_end[:, i, :] = 1
