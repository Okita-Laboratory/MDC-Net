...
class MDM(nn.Module):
    def __init__(self, modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
                 ...
                 arch='trans_dec', emb_trans_dec=True, clip_version=None, **kargs):
        super().__init__()
    ...
    def forward(self, x, timesteps, mod=None):
        """
        x: [batch_size, nfeats, max_frames], denoted x_t in the paper
        timesteps: [batch_size] (int)
        nfeats = njoints * 3
        """
        # print('x', x.shape) [64, 20, 48]
        ...
        if self.arch == 'gru':
            x_reshaped = x.reshape(bs, nfeat, 1, nframes)
        ...
        # print('output',output.shape)  [64, 16, 3, 20]
        # 改变他的维度，合并1,2维度，并与0维度交换位置
        output = output.permute((0, 3, 1, 2)).view(B, 20, 48)
...
if __name__ == '__main__':
    ...
    # print(model)
    # N, t_his + t_pre + t_fut, joints, 3
    x = torch.randn((8, 20, 48))
