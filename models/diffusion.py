...
class Diffusion:
    ...
    def noise_motion(self, x, t):
        ...
        # sqrt_alpha_hat * x + sqrt_one_minus_alpha_hat * Ɛ   function(3) in paper
    ...
    def inpaint_complete(self, step, x, prev_t, traj_dct, mode_dict):
        ...
        x_prev_t_unknown = torch.matmul(self.idct[:, :self.n_pre],
                                        x[:, :self.n_pre])
        """
        这个操作将掩码与x_prev_t_known相乘。掩码中值为1的位置会保留x_prev_t_known中对应位置的值，而值为0的位置会变为0。
        通过(1 - mode_dict['mask'])计算掩码的反转（invert the mask），使得原本为1的部分变为0，原本为0的部分变为1。
        然后，将这个反转后的掩码与x_prev_t_unknown相乘。这样，x_prev_t_unknown中原本被掩码“遮盖”的部分现在会被保留下来。
        这是论文中将俩段序列合并的操作
        """
        ...
        if mode_dict['mode'] == 'joint':
            traj_dct_joint = torch.matmul(self.dct[:self.n_pre], mode_dict['traj_joint'])
            x_prev_t_end_known, _ = self.noise_motion(traj_dct_joint, prev_t)
            x_prev_t_end_known = torch.matmul(self.idct[:, :self.n_pre], x_prev_t_end_known[:, :self.n_pre])
            x = torch.mul(mode_dict['mask_joint'], x_prev_t_end_known) + torch.mul((1 - mode_dict['mask_joint']), x)
