...
from models.FlowMDM.x_transformers.attend import Attend, Intermediates, CascadingHeads
...
class BidirectionalAlibiPositionalBias(AlibiPositionalBias):
    ...
    def forward(self, i, j):
        ...
        # for efficiency
        if exists(self.bias) and self.bias.shape[-1] >= j and self.bias.shape[
            -2] >= i:  # if previous adequate bias is buffered
            return self.bias[..., :i, :j]
...
class BPE_AbsoluteSinusoidal(nn.Module):
    ...
    def _forward_multisegment(self, seq_len, device, timesteps, pos_pe_abs, training, pe_bias, **kwargs):
        ...
        if training or (not exists(self.freqs_abs_multi) or self.last_timestep < timesteps[0]):
            ...
            sel_freqs_abs = torch.gather(freqs_abs_rep, dim=1, index=pos_pe_abs.to(int).unsqueeze(-1).expand(-1, -1,
                                                                                                             self.ape_dim))  # select according to those indices
...
class Attention(nn.Module):
    def __init__(
        ...
        if self.max_attend_past != self.max_attend_future:
            print(
                "[WARNING] Local relative attention is not symmetric. This can cause problems due to chunked attention.")
    ...
    def forward(
        ...
        if exists(rotary_pos_emb):  # and not has_context:
            ...
            (ql, qr), (kl, kr), (vl, vr) = map(lambda t: (t[..., :l], t[..., l:]),
                                               (q, k, v))  # split query, key, value into rotary and non-rotary parts
            ...
            q, k, v = map(lambda t: torch.cat(t, dim=-1),
                          ((ql, qr), (kl, kr), (vl, vr)))  # concat query, key, value rotary and non-rotary parts
...
class AttentionLayers(nn.Module):
    def __init__(
        ...
        if rotary_bpe_schedule is None and rotary_pos_emb:
            ...
            self.rotary_pos_emb = BPE_Rotary(rotary_bpe_schedule, rotary_emb_dim, use_xpos=rotary_xpos,
                                             scale_base=rotary_xpos_scale_base,
                                             interpolation_factor=rotary_interpolation_factor,
                                             base_rescale_factor=rotary_base_rescale_factor)
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type in ['a', 'A']:
                ...
                layer = Attention(dim, heads=heads, causal=causal,
                                  custom_query_fn=custom_query_fn[a_idx] if isinstance(custom_query_fn, (
                                  list, tuple)) else custom_query_fn,
                                  **attn_kwargs)
        ...
        for ind, (layer_type, (norm, block, residual_fn), layer_dropout) in enumerate(
            ...
            if layer_type in ['a', 'A']:
                out, inter = block(x, mask=mask, context_mask=self_attn_context_mask, attn_mask=attn_mask,
                                   ...
                                   mem=layer_mem, cond_tokens=cond_tokens, attn_bias=attn_bias,
                                   chunked_attn=chunked_attn)
            elif layer_type in ['c', 'C']:
                _x, _context = (x, context) if layer_type == 'c' else (context, x)  # else is custom crossattention
                out, inter = block(_x, context=_context, mask=mask, context_mask=context_mask,
                                   prev_attn=prev_cross_attn, cond_tokens=cond_tokens, chunked_attn=chunked_attn)
