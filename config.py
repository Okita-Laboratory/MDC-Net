...
def update_config(cfg, args_dict):
    ...
    cfg.dct_m, cfg.idct_m = util.get_dct_matrix(cfg.t_pred + cfg.t_his + cfg.t_fut)
...
class Config:
    ...
    def __init__(self, cfg_id, test=False):
        self.id = cfg_id
        cfg_name = '/home/ghy/workspace/MOCAP/myHumanMAC/cfg/%s.yml' % cfg_id
        ...
        """
        这行代码是在从配置文件中获取名为 'normalize_data' 的配置参数，
        如果配置文件中不存在 'normalize_data' 这个参数，则返回默认值 False。
        """
        self.normalize_data = cfg.get('normalize_data', False)
        self.t_his = cfg['t_his']
        self.t_pred = cfg['t_pred']
        self.t_fut = cfg['t_fut']
        ...
        self.idx_pad, self.zero_index = generate_pad(self.padding, self.t_his, self.t_pred, self.t_fut)