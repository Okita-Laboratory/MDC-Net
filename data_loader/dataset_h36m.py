...
class DatasetH36M(Dataset):
    ...
    def __init__(self, mode, t_his=25, t_pred=100, t_fut=25, actions='all', use_vel=False):
    ...
    def prepare_data(self):
        ...
        self.subjects_split = {'train': [1, 5, 6, 7, 8],
                               'test': [9, 11]}
        """
        if subjects==train [1, 5, 6, 7, 8]
        if subjects==test [9, 11]
        """
...
if __name__ == '__main__':
    np.random.seed(0)
    actions = {'Phoning'}
    dataset = DatasetH36M('test', actions=actions)
