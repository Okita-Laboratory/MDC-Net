...
class DatasetH36M_multi(Dataset):
    ...
    def __init__(self, mode, t_his=25, t_pred=100, t_fut=25, actions='all', use_vel=False, **kwargs):
        ...
        super().__init__(mode, t_his, t_pred, t_fut, actions)
        if use_vel:
            self.traj_dim += 3

    def prepare_data(self):
        self.data_file = os.path.join('/home/ghy/workspace/MOCAP/myHumanMAC/data', 'data_3d_h36m.npz')
    ...
    def sample(self, n_modality=5):
        ...
        if n_modality > 0 and subject in self.data_multimodal.keys():
            ...
            if len(traj_multi) > 0:
                ...
                traj_multi[:, self.t_fut:] = traj[None, ...][:, self.t_fut:]
            ...
            for act in data_s.keys():
                ...
                for i in range(0, seq_len - self.t_total, step):
                    ...
                    if n_modality > 0:
                        ...
                        # print(f'candi_tmp.shape={candi_tmp.shape}')
                        # print(f'x0[:, self.t_his - margin_f:self.t_his, 1:].shape={x0[:, self.t_his - margin_f:self.t_his, 1:].shape}')
                        # print(f'candi_tmp[:, self.t_his - margin_f:self.t_his, 1:].shape={candi_tmp[:, self.t_his - margin_f:self.t_his, 1:].shape}')
