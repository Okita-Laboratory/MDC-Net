...
class Dataset:
    ...
    def __init__(self, mode, t_his, t_pred, t_fut, actions='all'):
        self.mode = mode
        self.t_his = t_his
        self.t_pred = t_pred
        self.t_fut = t_fut
        self.t_total = t_his + t_pred + t_fut
    ...
    def my_sample_iter_action(self, action_category, dataset_type, sample_mode='last'):
        # subject = np.random.choice(self.subjects)
        if dataset_type == 'h36m':
            dict_s = self.data['S9']
        elif dataset_type == 'humaneva':
        ...
        action = action_category
        seq = dict_s[action]
        # 根据 sample_mode 来确定 fr_start 和 fr_end
        if sample_mode == 'first':
            fr_start = 200
            # print('first, fr_start: ', fr_start)
        elif sample_mode == 'last':
            fr_start = seq.shape[0] - self.t_total
            # print('last, fr_start: ', fr_start)
        else:
            raise ValueError("sample_mode must be either 'first' or 'last'")
    ...
    def my_sample_action(self, action_category, dataset_type, sample_mode, start, end):
        # subject = np.random.choice(self.subjects)
        if dataset_type == 'h36m':
            dict_s = self.data['S9']
        elif dataset_type == 'humaneva':
        ...
        if sample_mode == 'all':
            ...
            if end == 'end':
                ...
                if end > seq.shape[0]:
                    raise ValueError(f'{end} is out of {seq.shape[0]}, please input one number smaller than {seq.shape[0]}')
                fr_end = end

            traj = seq[fr_start: fr_end]

            print(f'The used frame of {action} is from {fr_start} to {fr_end}')
        else:
            raise ValueError("sample_mode must be either 'all' or 'part'")
